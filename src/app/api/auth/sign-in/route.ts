...
import { signInWithCode } from '@/lib/telegram/auth';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await signInWithCode(client, validated.phoneNumber, validated.code, validated.phoneCodeHash);
    ...
    if (!result.success || !result.user) {
      return Response.json({ success: false, error: 'Sign in failed' }, { status: 400 });
    }
